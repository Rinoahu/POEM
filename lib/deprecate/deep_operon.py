...
class CNN:
    ...
    def fit_2d(self, X_train, y_train, X_test=None, y_test=None):
        ...
        # set the conv model
        model = Sequential()
        model.add(Convolution2D(nb_filters, (nb_conv, nb_conv), border_mode='same', input_shape=(
            b, img_rows, img_cols), activation='relu', name='conv1_1', data_format='channels_first'))

        model.add(Convolution2D(64, (3, 3), activation='relu',
                                name='conv1_2', data_format='channels_first'))
        model.add(MaxPooling2D((2, 2), strides=(2, 2)))

        model.add(Convolution2D(128, (3, 3), activation='relu',
                                name='conv2_1', data_format='channels_first'))
        model.add(MaxPooling2D((2, 2), strides=(2, 2)))
    ...
    def load(self, name, mode='2d'):
        model = keras.models.load_model(
            name, custom_objects={'f1_score': f1_score, 'tf': tf, 'fbeta_score': f1_score})
...
# run training
def run_train(train, seq_dict, clf, mode='2d', rounds=5):
    # get the training data
